class SVG {
  ...
  void beginSVG() {
    ...
    file=append(file, "<svg xmlns:xlink='http://www.w3.org/1999/xlink' style='fill-opacity:1; color-rendering:auto; color-interpolation:auto; text-rendering:auto; stroke:black; stroke-linecap:square; stroke-miterlimit:10; shape-rendering:auto; stroke-opacity:1; fill:black; stroke-dasharray:none; font-weight:normal; stroke-width:1; font-family:"+"Arial"+"; font-style:normal; stroke-linejoin:miter; font-size:12px; stroke-dashoffset:0; image-rendering:auto;' width='"+filewidth+"' height='"+fileheight+"' xmlns='http://www.w3.org/2000/svg'");
    file=append(file, "");
        ...
        for (Point x : p.ps) {
          for (Line k : x.ls) {
            if (k.exselected&&x.selected) {
              file=append(file, "<line style='stroke-width:2;' stroke-dasharray='10' x1='"+toAbs(k.p[1].x, 0)+"' y1='"+toAbs(k.p[1].y, 1)+"' x2='"+toAbs(k.p[0].x, 0)+"' y2='"+toAbs(k.p[0].y, 1)+"'/>");
            }
          }
        }
        if (l.exselected&&p.selected) {
          file=append(file, "<line style='stroke-width:2;' stroke-dasharray='10' x1='"+toAbs(l.p[1].x, 0)+"' y1='"+toAbs(l.p[1].y, 1)+"' x2='"+toAbs(l.p[0].x, 0)+"' y2='"+toAbs(l.p[0].y, 1)+"'/>");
        }
      }
    }
    file=append(file, "</g>");
  ...
  void renderText() {
    file=append(file, "<g style='stroke-width:0;' fill = 'black' font-size = '20' font-family='Arial' alignment-baseline='hanging'>");

    for (TextBox tb : tbs) {
      file=append(file, "<text x = '"+toAbs(tb.x+7, 0)+"' y = '"+toAbs(tb.y-18, 1)+"' >");
      file=append(file, "<tspan>");
      file=append(file, tb.t[0]);
      file=append(file, "</tspan>");
